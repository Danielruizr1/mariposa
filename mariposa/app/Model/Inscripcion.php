 ...
 */
   ...
   public function salvabitacora($iduser,$idseguimiento,$texto, $fecha, $hora){
	   $this->query("insert into bitacora2 (seguimientos_id,users_id,ingreso,fecha,hora) values ('$idseguimiento', '$iduser', '$texto', '$fecha', '$hora')");
	   $id=$this->query("SELECT MAX(id) as id FROM bitacora2");
	   return ($id[0][0]['id']);
   }
   ...
   public function transferir($idusuario, $idseguimiento, $idantiguo, $nombreusuarionuevo, $idusuarioantiguo){
	   $fecha=date("Y-m-d");
	   $hora=date("H:m:s");
	   $this->query("update inscripciones set users_id=$idusuario, transferencia=1 where id=$idseguimiento");
	   $this->query("insert into bitacora values ('', $idseguimiento, $idusuarioantiguo, '$idantiguo ha transferido el seguimiento actual a $nombreusuarionuevo', '$fecha', '$hora')");
   ...
   public function agregarAgencia($medio){
	   $this->query("insert into agencias values ('', '$medio', 1, '', 1, 1, '', '', '', '', '', '', '', '', '', '', '')");
